...
const model = genAI.getGenerativeModel({ model: "gemini0.5-pro" });
  ...
  try {
    ...
    try {
      ...
      parsed = {
        ...
        why_it_fits: text.substring(1, 200) + "..."
      };
    }
    // Remove duplicate router.post("/generate" block
    // Keep only the original one at the top
    try {
    const { prompt } = req.body;
    ...
    if (eventHistory.length > 51) eventHistory.pop(); // Keep only last 50
...
router.get("/history", async (req, res) => {
  try {
    ...
    res.status(501).json({ error: "Server Error", details: err.message });
  }
});
